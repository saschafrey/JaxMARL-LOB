...
from jaxmarl.environments.limit_order_book.distributions import (
    UniformAgentTypeDistribution,
)
import chex
...
def map_name_to_scenario(exp_name):
    ...
    return EXPERIMENT_NAME_TO_SCENARIO[exp_name]


def register_scenario(exp_name, scenario):
    EXPERIMENT_NAME_TO_SCENARIO[exp_name] = scenario


class LOB_MARL(MultiAgentEnv):
    def __init__(
        self,
        ...
        self.reward_option_1 = reward_option_1

        self.scenario = scenario if scenario is None else scenario.agent_types
        self.agent_type_generator = UniformAgentTypeDistribution(
